...
export default async function handler(req, res) {
  ...
  try {
    // Get non-subscriber conversations only (users without active subscription)
    const conversations = db.prepare(`
      ...
      WHERE u.subscription_status != 'active' OR u.subscription_status IS NULL
      GROUP BY sm.user_id
    ...
    console.error('Failed to fetch non-subscriber conversations:', error);
    return res.status(500).json({ error: 'Failed to fetch non-subscriber conversations' });
  }
}
