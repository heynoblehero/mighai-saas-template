...
export default function NewBackendRoute() {
  const [formData, setFormData] = useState({
    name: '',
    slug: '',
    method: 'GET',
    description: '',
    code: `// Your route handler code here
// Available objects: req, res, console
// Use require() to load npm packages
...
try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Auto-install packages if any were specified
        if (formData.packages.length > 0) {
          setError('Installing packages... This may take a minute.');

          try {
            const installResponse = await fetch(`/api/admin/custom-routes/${routeId}/install-packages`, {
              method: 'POST'
            });
            ...
            if (!installResponse.ok) {
              ...
              setError(`Route created, but package installation failed: ${installData.error || installData.message}. You can install packages manually from the routes list.`);
              setTimeout(() => router.push('/admin/backend/routes'), 3000);
              return;
            }
          } catch (installErr) {
            console.error('Package installation error:', installErr);
            setError('Route created, but package installation failed. You can install packages manually from the routes list.');
            setTimeout(() => router.push('/admin/backend/routes'), 3000);
            return;
          }
        }
  ...
  return (
    <AdminLayout title="Create New Backend Route">
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-slate-100">Create New Backend Route</h1>
          <p className="text-slate-400 mt-1">Build a custom API endpoint with Node.js</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-700">
              ...
              <p className="text-sm text-slate-400 mt-1">
                Add npm packages that your route needs
              </p>
            </div>
