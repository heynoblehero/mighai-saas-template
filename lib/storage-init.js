...
function initializeStorageTables() {
  try {
    ...
    // Create default bucket for subscriber uploads
    const existingBucket = db.prepare('SELECT id FROM storage_buckets WHERE slug = ?').get('user-uploads');
    if (!existingBucket) {
      db.prepare(`
      ...
      `).run(
        'User Uploads',
        'user-uploads',
        'Default bucket for subscriber file uploads. Subscribers can upload images, documents, and other allowed files here.',
        JSON.stringify(['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'application/pdf', 'text/plain', 'application/json']),
        10485760, // 10MB
        'user'
      );
      console.log('✓ Default user-uploads bucket created');
    }
