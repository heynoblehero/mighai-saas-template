...
const FORBIDDEN_TAGS = [
  ...
  'base', 'meta', 'noscript', 'frameset', 'frame'
];
...
export function sanitizeHTML(html) {
  ...
  try {
    ...
    const suspiciousPatterns = [
      ...
      /vbscript:/i,
      /data:text\/html/i,
      /on\w+\s*=/i  // Catch any remaining event handlers
    ];

    for (const pattern of suspiciousPatterns) {
      if (pattern.test(clean)) {
        console.warn('[SANITIZER] Suspicious pattern detected after sanitization:', pattern);
...
export function validateHTMLSafety(html) {
  ...
  const dangerousPatterns = [
    ...
    { pattern: /<link/i, message: 'Link tags are not allowed (use inline styles instead)' },
    { pattern: /<style/i, message: 'Style tags should be in separate CSS file' },
